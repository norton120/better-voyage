...
- **Persisting per-stage progress history** (for a "where did the
  ...
  from OTel traces.
- **Priority queue** (e.g. interactive runs over batch): not MVP,
  single-user local tool.
- **Redis-backed arq upgrade** if we ever grow to multi-worker:
  straightforward — `JobRegistry` becomes a thin shim and the DB
  source-of-truth pattern carries over.
- **Resumable stages** (don't re-download charts if a fetch was
  90% complete and the process died): defer until it bites.
